...
Функция РаспределитьРасходыНаФинансовыйРезультат(Период, Организации, МенеджерВременныхТаблиц = Неопределено, ДокументРаспределения = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		//////////////////////////////////////////////////////////////////////
		// К распределению суммы ПР и ВР, при выключенном использовании РУ,
		// применяется простая логика трансляции разниц из остатков - 
		// все образованные разницы распределяются документом 
		// с флажком РегламентированныйУчет без разделения на БУ и НУ
		//////////////////////////////////////////////////////////////////////

		|ВЫБРАТЬ //**02 - при вкл. ФО ИспользоватьРеглУчет отключаем выборку
		|	ОстаткиРасходов.Организация КАК Организация,
		|	ОстаткиРасходов.Подразделение КАК Подразделение,
		|	ОстаткиРасходов.НаправлениеДеятельности КАК НаправлениеДеятельности,
		|	ОстаткиРасходов.СтатьяРасходов КАК СтатьяРасходов,
	...
	Если ИспользоватьРеглУчет Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "//**02", "ПЕРВЫЕ 0 //"); //@Query-part
	Иначе
