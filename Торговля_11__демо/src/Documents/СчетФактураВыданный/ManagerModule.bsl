...
Функция ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Иначе
		...
		Товары = РезультатПоТабличнойЧасти.Выгрузить().Скопировать(,"Ссылка,Номенклатура,Характеристика,Количество");
		Товары.Колонки.Характеристика.Имя = "временная";
		...
		Для Индекс = 1 По КоличествоСтрок Цикл
			Если Не ЗначениеЗаполнено(Товары[КоличествоСтрок-Индекс].Номенклатура) Тогда 
				Товары.Удалить(КоличествоСтрок-Индекс);
			Иначе 
				Товары[КоличествоСтрок-Индекс].Характеристика = Товары[КоличествоСтрок-Индекс].временная;
			КонецЕсли;
		КонецЦикла;
		Маркировка                = ЭлектронноеВзаимодействиеИСМП.ЧастичноеСодержимоеИКодыОСУ(
			Маркировка, Товары,	ШтрихкодированиеИС.ПараметрыСканирования(РезультатПоШапке.Выгрузить()[0].ДокументОснование));
