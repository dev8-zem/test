...
Процедура ПолучитьДанныеДокументаАккредитацииИзСервиса(ДанныеДокумента, ПараметрыДокумента)
	...
	Если Отказ Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Работа с номенклатурой.Получение данных сервиса ФСА'",
			ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , ПодробныйТекстОшибки);
	КонецЕсли;
...
Процедура ЗаполнитьДанныеСертификата(Приемник, Источник)
	...
	Если Источник.Свойство("certExpirationDate") Тогда
...
Процедура ЗаполнитьДанныеДекларации(Приемник, Источник)
	...
	Если Источник.Свойство("declExpirationDate") Тогда
