...
// 
// Параметры:
//  ВладелецФайла - ОпределяемыйТип.ВладелецФайлов - Ссылка объект-владелец файла
...
//
Функция ВизуализацияДокументаСШтампомЭП(ВладелецФайла, СсылкаНаДвоичныеДанныеФайла, Расширение) Экспорт
	...
	Если ДвоичныеДанныеФайлаPdf <> Неопределено Тогда
		Результат = ДобавитьШтампЭПВФайлPDF(ДвоичныеДанныеФайлаPdf, ВладелецФайла, Новый УникальныйИдентификатор);
	КонецЕсли;
...
//  СообщениеЭДО - ДокументСсылка.СообщениеЭДО - сообщение ЭДО для формирования данных по штампу ЭП
//  УникальныйИдентификатор - УникальныйИдентификатор
// 
// Возвращаемое значение:
//  Неопределено,Строка - адрес во временном хранилище
// 
Функция ДобавитьШтампЭПВФайлPDF(ДвоичныеДанныеФайлаPdf, СообщениеЭДО, УникальныйИдентификатор) Экспорт
...
#Если Не ВнешнееСоединение Тогда
	...
	ШтампКартинка = ЭлектронныеДокументыЭДО.ДанныеШтампаЭППоДаннымСообщенияЭДО(СообщениеЭДО);
	СмещениеОтКрая = 10;
	
	КодДляВыполнения = 
		"ЗаписьPDF = Новый ЗаписьPDF(ПотокДанныхФайла);
		|Описание = Новый ОписаниеОтображаемогоОбъектаPDF;
		|Описание.Объект = ШтампКартинка;
