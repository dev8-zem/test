...
// Загружает доверенность из файла, помещенного в харнилище.
...
//
// Параметры:
//   РезультатПодписания - Неопределено - будет прислано в случае прервыания подписания пользователем на форме ожидания,
...
Процедура ОбновитьСостояниеВРеестреФНС(
	...
	Если (ФормаДоверенности.Объект.СтатусВРеестреФНС <>
		...
		ИначеЕсли СведенияСтатусаДоверенности.СтатусДоверенности = "DECLINED"
			...
			ДатаОтзыва = МашиночитаемыеДоверенностиВызовСервера.ДатаОтзываДоверенности(ФормаДоверенности.Объект.НомерДоверенности,
...
Процедура ПолучитьДанныеМЧДПослеВводаРеквизитов(
	...
	ДанныеДляЗагрузки = МашиночитаемыеДоверенностиВызовСервера.НовыеДанныеДляЗагрузкиМЧД();
	ДанныеДляЗагрузки.ДанныеМЧД = ПолныеДанные.ДанныеВыгрузки;
	ДанныеДляЗагрузки.Подписи = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПолныеДанные.ДанныеПодписи);
