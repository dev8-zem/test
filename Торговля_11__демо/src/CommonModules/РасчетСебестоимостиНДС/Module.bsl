...
Процедура РаспределениеПартийНДСФИФОСкользящаяПоДопРасходам(ПараметрыРасчета) Экспорт
	...
	|ИЗ
	|(ВЫБРАТЬ
	// Таможенная декларация с доп. расходами по документу поступления прошлого периода должна определяться как переменные доп. расходы
	|	ВЫБОР
	...
	|				И ЕСТЬNULL(РеестрДокументаПоступления.ДатаДокументаИБ, &НачалоПериода) >= &НачалоПериода
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументаПоступления
	|		ПО РеестрДокументаПоступления.Ссылка = ДД.ДокументПоступления
	|		И НЕ РеестрДокументаПоступления.ДополнительнаяЗапись
	|		И РеестрДокументаПоступления.Ссылка <> НЕОПРЕДЕЛЕНО
	|		И ТИПЗНАЧЕНИЯ(ДД.ДокументПоступления) = ТИП(Документ.ТаможеннаяДекларацияИмпорт)
	|СГРУППИРОВАТЬ ПО
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДД.ДокументПоступления) = ТИП(Документ.ТаможеннаяДекларацияИмпорт)
	|				И ЕСТЬNULL(РеестрПартия.ДатаДокументаИБ, &КонецПредыдущегоПериода) < &НачалоПериода
...
Функция ТекстЗапросаДляПартийНДС(ПараметрыРасчета = Неопределено) Экспорт
	...
	ТекстЗапроса = ""
		...
		+ ОбщегоНазначенияУТ.РазделительЗапросовВПакете() + ТекстАналитикиУчетаПартийТаможенныхДеклараций() // ВТИзменениеВидаДеятельностиДопРасходов
...
Процедура СформироватьСтоимостьПартийНДСНаКонецПериода(ПараметрыРасчета, НомерВызова)
	...
	// Заполнение поля ЭтоДанныеДляПартийНДСКРаспределению должно соответствовать условиям в ТекстПоступленияПартийНДСКРаспределениюИзПартийНДС2_4()
...
Функция ТекстИнициализацияРаспределениеПартийНДСФИФОСкользящая()
	Возврат
		...
		|	Т.ИдентификаторФинЗаписи,
		|	Т.ХозяйственнаяОперация,
		|	СУММА(ВЫБОР КОГДА ВЫРАЗИТЬ(Т.Количество * Партии.СтоимостьБезНДСРегл * Статьи.КоэффициентРегл КАК ЧИСЛО (31,2)) > Т.Количество * Партии.СтоимостьБезНДСРегл * Статьи.КоэффициентРегл
		|			И Т.Количество * Партии.СтоимостьБезНДСРегл * Статьи.КоэффициентРегл > 0
		|			И НЕ ОСНО.СистемаНалогообложения ЕСТЬ NULL
		|		ТОГДА ВЫРАЗИТЬ(Т.Количество * Партии.СтоимостьБезНДСРегл * Статьи.КоэффициентРегл КАК ЧИСЛО (31,2)) - 0.01
		|		ИНАЧЕ ВЫРАЗИТЬ(Т.Количество * Партии.СтоимостьБезНДСРегл * Статьи.КоэффициентРегл КАК ЧИСЛО (31,2))
		|	КОНЕЦ) КАК СтоимостьБезНДС,
		|	СУММА(ВЫБОР КОГДА ВЫРАЗИТЬ(Т.Количество * Партии.НДСРегл * Статьи.КоэффициентРегл КАК ЧИСЛО (31,2)) > Т.Количество * Партии.НДСРегл * Статьи.КоэффициентРегл
		|			И Т.Количество * Партии.НДСРегл * Статьи.КоэффициентРегл > 0
		|			И НЕ ОСНО.СистемаНалогообложения ЕСТЬ NULL
		|		ТОГДА ВЫРАЗИТЬ(Т.Количество * Партии.НДСРегл * Статьи.КоэффициентРегл КАК ЧИСЛО (31,2)) - 0.01
		|		ИНАЧЕ ВЫРАЗИТЬ(Т.Количество * Партии.НДСРегл * Статьи.КоэффициентРегл КАК ЧИСЛО (31,2))
		|	КОНЕЦ) КАК НДС,
		|	СУММА(ВЫБОР КОГДА ВЫРАЗИТЬ(Т.Количество * Партии.СтоимостьБезНДСУпр * Статьи.КоэффициентУпр КАК ЧИСЛО (31,2)) > Т.Количество * Партии.СтоимостьБезНДСУпр * Статьи.КоэффициентУпр
		|			И Т.Количество * Партии.СтоимостьБезНДСУпр * Статьи.КоэффициентУпр > 0
		|			И НЕ ОСНО.СистемаНалогообложения ЕСТЬ NULL
		|		ТОГДА ВЫРАЗИТЬ(Т.Количество * Партии.СтоимостьБезНДСУпр * Статьи.КоэффициентУпр КАК ЧИСЛО (31,2)) - 0.01
		|		ИНАЧЕ ВЫРАЗИТЬ(Т.Количество * Партии.СтоимостьБезНДСУпр * Статьи.КоэффициентУпр КАК ЧИСЛО (31,2))
		|	КОНЕЦ) КАК СтоимостьБезНДСУпр,
		|	СУММА(ВЫБОР КОГДА ВЫРАЗИТЬ(Т.Количество * Партии.НДСУпр * Статьи.КоэффициентУпр КАК ЧИСЛО (31,2)) > Т.Количество * Партии.НДСУпр * Статьи.КоэффициентУпр
		|			И Т.Количество * Партии.НДСУпр * Статьи.КоэффициентУпр > 0
		|			И НЕ ОСНО.СистемаНалогообложения ЕСТЬ NULL
		|		ТОГДА ВЫРАЗИТЬ(Т.Количество * Партии.НДСУпр * Статьи.КоэффициентУпр КАК ЧИСЛО (31,2)) - 0.01
		|		ИНАЧЕ ВЫРАЗИТЬ(Т.Количество * Партии.НДСУпр * Статьи.КоэффициентУпр КАК ЧИСЛО (31,2))
		|	КОНЕЦ) КАК НДСУпр
	...
	Если Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.БазаРегл
	 ИЛИ Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.БазаУпр Тогда
		ЗаполнитьЗначенияСвойств(РасчетнаяПартия, Расход, "СтатьяКалькуляции, ГруппаПродукции, ПравилоОтнесенияНаВыпуск,
			|КорПодразделение, КорНаправлениеДеятельности, ИдентификаторФинЗаписи, НаВыбытие");
	КонецЕсли;
...
Функция ТекстЗапросаДетализацияПартийНДС(ТекстОрганизация = "", ТекстНачалоПериода = "", ТекстКонецПериода = "", СтруктураОтбора = Неопределено, СтруктураДанных = Неопределено) Экспорт
	...
	|		И Т.Месяц >= УчетныеПолитики.Период
	|СГРУППИРОВАТЬ ПО
	|	Т.Месяц,
	|	Т.Организация,
	|	Т.ДокументПоступления
	...
	|ГДЕ
	|	НЕ УчетныеПолитики.ВариантУчетаНДСПриИзмененииВидаДеятельности ЕСТЬ NULL
	|	И УчетныеПолитики.ВариантУчетаНДСПриИзмененииВидаДеятельности <> ЗНАЧЕНИЕ(Перечисление.ВариантыУчетаНДСПриИзмененииВидаДеятельностиНаНеоблагаемую.ВключатьВСтоимость)
	...
	|		ПО Т.Организация = УчетнаяПолитика.Организация
	|		И Т.Месяц = УчетнаяПолитика.Месяц
	|		И Т.ДокументПоступления = УчетнаяПолитика.ДокументПоступления
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ДанныеПервичныхДокументов.Организация = Т.Организация
	|		И ДанныеПервичныхДокументов.Документ = Т.ДокументПоступления
	|ГДЕ
