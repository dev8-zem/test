...
Функция ДатаОтзываДоверенности(НомерДоверенности, ДоверительЮЛ_ИНН, ДоверительФЛ_ИНН, ТокенДоступа) Экспорт
	...
	Если Не СведенияОДоверенности.Ошибка 
		И СведенияОДоверенности.ПолныеДанные.ДанныеПодписиЗаявленияНаОтмену <> Неопределено Тогда
			ДатаОтзыва = 
				ЭлектроннаяПодпись.ДатаПодписания(СведенияОДоверенности.ПолныеДанные.ДанныеПодписиЗаявленияНаОтмену);
	КонецЕсли;
...
Функция ВерсияФорматаОбъектаМЧД(ДанныеXDTO) Экспорт
	...
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеXDTO, "ВерсФорм") = "EMCHD_1" Тогда
...
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//  * Ключ - СправочникСсылка.МашиночитаемыеДоверенностиОрганизаций,СправочникСсылка.МашиночитаемыеДоверенностиКонтрагентов,СправочникСсылка.МЧД003
//  	   - СправочникСсылка.МЧД003
...
Функция ТабличныйДокументМЧД(ДвоичныеДанные, ЭтоМЧДКонтрагента = Ложь) Экспорт
	...
	Если РезультатЧтения.ЭтоОбъектXDTO Тогда
		...
		ТабличныйДокумент = ТабличныйДокументПоФормату(РезультатЧтения.ВерсияФорматаМЧД, РезультатЧтения.ДанныеМЧД);
	...
	Попытка
		Для Каждого Доверенность Из Доверенности Цикл
			...
			Если ДанныеДоверенности.ЭтоФайловаяДоверенность Тогда
				...
				ФайлыМЧД = Новый Структура();
				ФайлыМЧД.Вставить("ДанныеМЧД", ДанныеДоверенности.Доверенность.ДвоичныеДанные);
					
				ВерсияМЧД = ВерсияФорматаОбъектаМЧД(Результат.ДанныеМЧД);
				Если ВерсияМЧД = МашиночитаемыеДоверенностиКлиентСервер.ФорматМЧД_003() Тогда
					ФайлыМЧД.Вставить("ДанныеМЧД", ДанныеДоверенности.Доверенность.ДвоичныеДанные);
					ФайлыМЧД.Вставить("Подписи",
						ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДанныеДоверенности.Подпись.ДвоичныеДанные));
					Результат = Справочники.МЧД003.ЗагрузитьМЧДИзФайла(ФайлыМЧД);
				Иначе
					ФайлыМЧД.Вставить("ДанныеПодписи", ДанныеДоверенности.Подпись.ДвоичныеДанные);
...
//    ** Значение - см. МашиночитаемыеДоверенности.НоваяОшибкаПроверкиПолномочийПриПодписании
//
Функция ПолучитьПоследнююВыданнуюДоверенностьB2B(Отбор, ЭлектронныйДокумент) Экспорт
...
//  * ДанныеМЧД - Неопределено, ОбъектXDTO, Структура - Данные доверенности, см. НовыеДанныеМЧД
//  * ВерсияФорматаМЧД - Строка
...
Функция ДанныеИзФайлаОбмена(ВходящиеДанные, ЭтоДоверенностьОрганизации = Неопределено) Экспорт
	...
	Результат.Вставить("ТекстОшибки", "");
	Результат.Вставить("ДанныеМЧД", Неопределено);
...
Процедура ЗаписатьМашиночитаемуюДоверенностьВЖурнал(Источник, Отказ) Экспорт
	...
	Если НЕ Источник.ДополнительныеСвойства.Свойство("НеЗаписыватьВЖурнал") Тогда
...
Функция ПолучитьПараметрыЗаписиВЖурналМЧД(Источник) Экспорт
...
Функция ЗагрузитьМЧДИзФайла(АдресХранилища, ТребуетсяПроверкаМЧДНаКлиенте = Ложь, ОбновлятьСуществующий = Ложь,
	...
	Иначе
		Данные = ПрочитатьАрхивСДоверенностьюИПодписью(АдресХранилища);
	КонецЕсли;
	...
	РезультатЧтения = ПрочитатьДанныеXML(Данные.ДанныеМЧД);
	...
	ЭтоФормат003 = 
		ВерсияФорматаОбъектаМЧД(РезультатЧтения.ДанныеМЧД) = МашиночитаемыеДоверенностиКлиентСервер.ФорматМЧД_003();

	Если ЭтоФормат003 Тогда
		Возврат Справочники.МЧД003.ЗагрузитьМЧДИзФайла(АдресХранилища,, ДополнительныеСведения); 
	Иначе
		
		ИНН = ИННДоверителей(Данные.ДанныеМЧД);
		
		ИННОрганизации = ИНН.ИННЮЛ;
		Если Не ЗначениеЗаполнено(ИНН.ИННЮЛ)
			И ЗначениеЗаполнено(ИНН.ИННФЛ) Тогда
			ИННОрганизации = ИНН.ИННФЛ;
		КонецЕсли;
		
		Организация = ОбщегоНазначенияБЭД.НайтиСсылку("Организации", , Новый Структура("ИНН", ИННОрганизации));
...
// * ДанныеПодписиЗаявленийНаОтмену - ДвоичныеДанные
Функция НовыеДанныеДляЗагрузкиМЧД() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ДанныеМЧД", ПустыеДвоичныеДанные());
	Результат.Вставить("Подписи", Новый Массив());
	Результат.Вставить("ДанныеПодписиЗаявленияНаОтмену", ПустыеДвоичныеДанные());
...
Функция ПрочитатьАрхив(ДанныеАрхива) Экспорт
	...
	ИначеЕсли ТипЗнч(ДанныеАрхива) = Тип("Структура") Тогда
...
Функция ПолучитьРодительскиеДанныеДоверенности(НомерДоверенности,
	...
	Если ЗначениеЗаполнено(СведенияДоверенности.ПолныеДанные) Тогда
		...
		ДанныеДляЗагрузки = НовыеДанныеДляЗагрузкиМЧД();
		ДанныеДляЗагрузки.ДанныеМЧД = СведенияДоверенности.ПолныеДанные.ДанныеВыгрузки;
		ДанныеДляЗагрузки.Подписи.Добавить(СведенияДоверенности.ПолныеДанные.ДанныеПодписи);
...
Функция ПроверитьПолномочияДоверенности(Доверенность, ЭлектронныйДокумент) Экспорт
	...
	Иначе
		...
		Если ТипДоверенности = Тип("СправочникСсылка.МашиночитаемыеДоверенностиКонтрагентов") Тогда
			...
			Если Не РезультатЧтения.Успех Или РезультатЧтения.ДанныеМЧД = Неопределено Тогда
			...
			Полномочия = РезультатЧтения.ДанныеМЧД.Полномочия;
...
// Возвращаемое значение:
//  Строка
Функция ВерсияФорматаПоДаннымЭлектронногоДокумента(ЭлектронныйДокумент) Экспорт
	...
	Если ЗначениеЗаполнено(ОсновнойФайл) Тогда
		...
		ДанныеАрхива = ПрочитатьАрхивСДоверенностьюИПодписью(ДанныеФайла.СсылкаНаДвоичныеДанныеФайла);
		Если ДанныеАрхива = Неопределено Тогда
			Возврат Результат;
		КонецЕсли;
		ДвоичныеДанныеМЧД = ДанныеАрхива.ДанныеМЧД;
...
Функция СведенияОСтатусахДоверенностей(СправочникМЧД, ДанныеМЧД)
	...
	Для Каждого ДанныеДоверенности Из ДанныеМЧД Цикл
		...
		Если ВерсияФорматаОбъектаМЧД(РезультатЧтения.ДанныеМЧД) = МашиночитаемыеДоверенностиКлиентСервер.ФорматМЧД_003() Тогда
...
Процедура ОбработатьДанные_ЗаполнитьТаблицуПредставителей(ОбъектМЧД, Записать)
	...
	ДанныеМЧД = ДанныеИзФайлаОбмена.ДанныеМЧД;
	
	Если ТипЗнч(ОбъектМЧД) = Тип("СправочникОбъект.МашиночитаемыеДоверенностиОрганизаций") Тогда
	
		Если ДанныеМЧД.Представители.Количество() > 0 Тогда
		
			Для Каждого Представитель Из ДанныеМЧД.Представители Цикл
	...
	ИначеЕсли ТипЗнч(ОбъектМЧД) = Тип("СправочникОбъект.МашиночитаемыеДоверенностиКонтрагентов") Тогда
		...
		Если ДанныеМЧД.Представители.Количество() > 0 Тогда	
			Для Каждого СведенияОПредставителе Из ДанныеМЧД.Представители Цикл
...
// Указывает вариант заполнения полномочий и заполнение реквизита представления полномочия в табличной части полномочий.
// 
// Параметры:
...
Функция ОбработатьДанные_ВозможноВыполнитьОбработкуМЧД(Объект)
	...
	Результат = Ложь;
	Если Не Объект.Подписана Тогда
		Возврат Результат;
	КонецЕсли;
	
	Если ТипЗнч(Объект) = Тип("СправочникОбъект.МашиночитаемыеДоверенностиОрганизаций")
		И (Не ПустаяСтрока(Объект.УдалитьПредставительЮЛ_ИНН)
		Или Не ПустаяСтрока(Объект.УдалитьПредставительФЛ_ИНН))
		И (Не ПустаяСтрока(Объект.ДоверительЮЛ_ИНН)
		Или Не ПустаяСтрока(Объект.ДоверительФЛ_ИНН)) Тогда
		Результат = Истина;
	ИначеЕсли ТипЗнч(Объект) = Тип("СправочникОбъект.МашиночитаемыеДоверенностиКонтрагентов") 
		И Не ПустаяСтрока(Объект.УдалитьПредставительИНН)
		И Не ПустаяСтрока(Объект.ДоверительИНН) Тогда
		Результат = Истина;
	КонецЕсли;
...
Процедура ВыполнитьДействияПослеПодписания(Доверенность, ДанныеВыгрузки, СвойстваПодписи, 
	...
	Попытка
		...
		ОбъектДоверенность.ДополнительныеСвойства.Вставить("НеЗаписыватьВЖурнал");
...
Функция ПрочитатьАрхивСДоверенностьюИПодписью(ДанныеАрхива) Экспорт
	...
	РезультатЧтения = ПрочитатьАрхив(ДанныеАрхива);
	
	Если НЕ ЗначениеЗаполнено(РезультатЧтения) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Если РезультатЧтения.Подписи.Количество() <> 1 Тогда
			
		ВидОперации = НСтр("ru = 'Чтение архива машиночитаемой доверенности'");
		ШаблонОшибки = НСтр("ru = 'Ошибка при чтении архива: %1'");
		
		Если РезультатЧтения.Подписи.Количество() = 0 Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Возврат Новый Структура("ДанныеМЧД, ДанныеПодписи", РезультатЧтения.ДанныеМЧД, РезультатЧтения.Подписи[0]);
...
Процедура ПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки) Экспорт
	Для Каждого СтрокаТаблицы Из Строки Цикл
		...
		Если СтрокаТаблицы.Значение.Данные.Количество > 1 Тогда
			...
			+ " и еще " + Строка(СтрокаТаблицы.Значение.Данные.Количество - 1);
		КонецЕсли;
...
Функция ТекстДекорацииИнформацииОДополнительнойПроверкеПолномочий() Экспорт
	Возврат "Дополнительные правила проверки полномочий позволяют расширить перечень разрешенных действий, которые определены автоматическими правилами.";
КонецФункции
...
Функция ЭтоМЧД003(СсылкаНаДоверенность)
	Возврат ТипЗнч(СсылкаНаДоверенность) = Тип("СправочникСсылка.МЧД003");
КонецФункции
