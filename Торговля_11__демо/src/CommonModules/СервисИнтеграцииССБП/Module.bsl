	...
	Для Каждого КлючЗначение Из ПлатежныеСистемыПрограммы Цикл
		...
		Если НастройкаСервис <> Неопределено И НастройкаСервис.Активен Тогда
			РезультатОперации.ДанныеУчастников.Вставить(
				...
				Новый Структура("АдресСтраницыПодключения, ТекстПредупреждения, БИК, Представление, АдресСтраницыЗаявки, ПартнераАгентаСБП",
					...
					НастройкаСервис.ПартнераАгентаСБП));
		КонецЕсли;
	КонецЦикла;
...
Функция ПлатежныеАгрегаторы() Экспорт
	...
	ПлатежныеАгрегаторы.Добавить(Перечисления.ПлатежныеСистемы.СБП_CDEKPay);
...
Функция ОперацияПолучениеНастроекУчастниковСБП()
	...
	Для Каждого ОписаниеНастройки Из ДанныеНастроек Цикл
		...
		Настройка.Вставить("ПартнераАгентаСБП",
