...
Процедура ПодменитьФайлНаФайлСВизуализацией(
	...
	АдресСШтампом = РаботаСФайламиБЭД.ДобавитьШтампЭПВФайлPDF(ДвоичныеДанныеФайлаPDF, 
		ДанныеФайла.Владелец, УникальныйИдентификатор);
	...
	Если Не ЗначениеЗаполнено(НомераМЧД) Тогда
		...
		Если ЗначениеЗаполнено(СтарыйРезультатПроверки) И ЗначениеЗаполнено(СтарыйРезультатПроверки.Доверенность) Тогда
			...
			Если ТипЗнч(СтарыйРезультатПроверки.Доверенность) = Тип("СправочникСсылка.МЧД003") Тогда
				ИННДоверителей = Справочники.МЧД003.ПолучитьИННДоверителей(СтарыйРезультатПроверки.Доверенность);
				ИННДоверителя = ?(ИННДоверителей.Количество(), ИННДоверителей[0], "");
			Иначе
				ЭтоМЧДКонтрагента = 
...
// Возвращает формат МЧД, переданный через доп. данные.
//
// Параметры:
//   ПараметрыВыполнения - см. ЭлектронныеДокументыЭДОКлиентСервер.НовыеПараметрыВыполненияДействийПоЭДО
//
// Возвращаемое значение:
//   Строка - формат доверенности
Функция ПолучитьФорматДоверенностиИзДополнительныхДанных(ПараметрыВыполнения, Доверенность)
	
	ФорматДоверенности = "";
	
	ДополнительныеДанные = ПараметрыВыполнения.ДополнительныеДанныеОбъектов.Получить(Доверенность);
	Если ДополнительныеДанные <> Неопределено Тогда
		ФорматИзДополнительныхДанных = ДополнительныеДанные.Получить("ФорматДоверенности");
		Если ФорматИзДополнительныхДанных <> Неопределено Тогда
		    ФорматДоверенности = ФорматИзДополнительныхДанных;
		КонецЕсли;	
	КонецЕсли;
	
	Возврат ФорматДоверенности;
...
Функция ЗапросДанныхСообщенийДляПодписания(ПараметрыВыполнения, КонтекстДиагностики)
	...
	Запрос.Текст = 
		...
		|	Ложь КАК ОбезличенныйСертификат
		|ИЗ
		|	ДоступныеСертификаты КАК ДоступныеСертификаты
...
Функция НайтиСоздатьНаборСертификатовДляПодписания(Выборка, НаборыСертификатов, СертификатыДляПодписания,
	...
	Если ИспользуютсяОбезличенныеСертификаты
		...
		СтрокиОбезличенногоСертификата = НайденныеСтроки.НайтиСтроки(Новый Структура("ОбезличенныйСертификат", Истина));
