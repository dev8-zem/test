...
Функция ОписаниеОбъектаУчета(ИсточникДанных) Экспорт
	...
	ИначеЕсли ТипИсточникаДанных = Тип("СправочникСсылка.МЧД003") Тогда
		...
		ИННДоверителя = Справочники.МЧД003.ПолучитьИННДоверителей(ИсточникДанныхСсылка);
		Организация = Справочники.МЧД003.ПолучитьОрганизациюДоверителяПоИНН(ИННДоверителя);
	...
	Для Каждого СтрокаПараметров Из ДанныеДокументов Цикл
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(ШаблонСообщения,
				СтрокаПараметров.ПредставлениеДокумента, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
