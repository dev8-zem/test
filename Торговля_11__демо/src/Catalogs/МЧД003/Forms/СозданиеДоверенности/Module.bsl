...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	Если Модифицированность И Не ЗавершениеРаботы Тогда
		Отказ = Истина;
		ТекстВопроса = НСтр("ru = 'Доверенность не была сохранена. Вы уверены, что хотите выйти?'");
...
Процедура ОбновитьВидимостьКомандТаблицыПредставителей(Форма)
	Форма.Элементы.ДобавитьПредставителя.Доступность = ЗначениеЗаполнено(
		Форма.Доверенность[0].Документ[0].Довер[0].СвУпПред[0]._Представитель);
	Форма.Элементы.ИзменитьПредставителя.Видимость = Форма.Доверенность[0].Документ[0].Довер[0].СвУпПред.Количество()
		> 1;
	Форма.Элементы.УдалитьПредставителя.Видимость = Форма.Доверенность[0].Документ[0].Довер[0].СвУпПред.Количество() > 1;
КонецПроцедуры
...
&НаКлиенте
Процедура Подключаемый_СброситьОформлениеЭлемента(Элемент) Экспорт
...
Процедура ЗаполнитьПредставителя(АдресХранилища, ИдентификаторСтроки)
	...
	Если ПредставительУжеУказан(ДанныеЗаполнения[0]._Представитель, ИдентификаторСтроки) Тогда
	...
	Иначе
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			Элементы.ДекорацияЗаголовокГруппыПредставителей.Заголовок = "Сведения о представителях";
			ОбновитьВидимостьКомандТаблицыПредставителей(ЭтотОбъект);
		КонецЕсли;
	КонецЕсли;
	ОбновитьНумерациюТаблицы(Доверенность[0].Документ[0].Довер[0].СвУпПред);
...
Функция ПредставительУжеУказан(Представитель, ТекущийИдентификатор)
	...
	СтруктураПоиска = Новый Структура("_Представитель", Представитель);
	НайденныеСтроки = ТаблицаПредставителей.НайтиСтроки(СтруктураПоиска);
	Для Каждого ПроверяемаяСтрока Из НайденныеСтроки Цикл
		Если ПроверяемаяСтрока.ПолучитьИдентификатор() = ТекущийИдентификатор Тогда
			Продолжить;
		КонецЕсли;
		Возврат Истина;
	КонецЦикла;
...
Процедура ПроверитьЗначениеРеквизита(Реквизит, Значение)
	...
	Если Фасет <> Неопределено Тогда
		...
		Если СтрДлина(Значение) <> ДопустимоеЗначениеРеквизита Тогда
	...
	Если Фасет <> Неопределено Тогда
		...
		Если ДопустимоеЗначениеРеквизита > 1 И СтрДлина(Значение) < ДопустимоеЗначениеРеквизита Тогда
...
// Временная замена платформеному механизму, до исправления ошибки.
// 
// Параметры:
